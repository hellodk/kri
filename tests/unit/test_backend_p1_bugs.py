...
def test_credential_resolver_has_logging():
    ...
    assert "warning" in src or "Warning" in src, "credential_resolver must log a warning on decryption failure"
...
def test_security_route_no_blocking_subprocess():
    ...
    assert "to_thread" in src, "security.py must use asyncio.to_thread to avoid blocking the event loop"
...
def test_alerts_route_no_blocking_urlopen():
    ...
    assert "to_thread" in src, "alerts.py must use asyncio.to_thread to avoid blocking the event loop"
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.AsyncFunctionDef, ast.FunctionDef)):
            if node.name == "list_models":
                ...
                assert "viewer" in func_src, "list_models must allow viewer role — model catalog is read-only"
                return
