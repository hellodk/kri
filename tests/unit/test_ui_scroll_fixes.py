...
def test_fleet_dashboard_thead_is_sticky():
    ...
    assert 'sticky' in thead_attrs, (
        "<thead> must have 'sticky' class — column headers scroll away without it (#363)"
    )
...
def test_node_detail_tab_bar_is_sticky():
    ...
    tab_bar_match = re.search(
        r'<div[^>]*(?:border-b|flex gap-1)[^>]*>.*?{tabs\.map',
        content, re.DOTALL
    )
    assert tab_bar_match, "Tab bar div not found in NodeDetail.tsx"
    tab_bar_html = tab_bar_match.group(0)
    assert 'sticky' in tab_bar_html, (
        "Tab bar div must have 'sticky' class (#364)"
    )
...
def test_playbook_job_detail_log_no_max_height():
    ...
    assert 'maxHeight' not in content, (
        "Log panel must not use maxHeight inline style — causes double scroll (#365). "
        "Use flex-1 overflow-auto instead."
    )
...
def test_playbook_job_detail_page_uses_flex_layout():
    ...
    assert 'flex-col' in content, (
        "PlaybookJobDetail wrapper must use flex-col layout (#365)"
    )
...
def test_fleet_dashboard_has_filter_toggle():
    ...
    content = open("frontend/src/pages/FleetDashboard.tsx").read()
    has_toggle = (
        'showMoreFilters' in content or
        'moreFilters' in content or
        'More filters' in content or
        'expandFilters' in content
    )
    assert has_toggle, (
        "FleetDashboard must have a secondary-filters toggle to reduce visual clutter (#366)"
    )
...
def test_fleet_dashboard_filter_count_badge():
    ...
    has_count = (
        'activeFilterCount' in content or
        'secondaryFilterCount' in content or
        'filterCount' in content or
        re.search(r'filter.*count|count.*filter', content, re.I) is not None
    )
    assert has_count, (
        "Toggle button must show count of active secondary filters (#366)"
    )
