"""Source-contract test: PROTECTED_TARGETS in NodeDetail.tsx must stay in sync
with PendingAction.PROTECTED_TARGETS in fleet_platform/models/pending_action.py.
...
FRONTEND_FILE = ROOT / "frontend" / "src" / "pages" / "NodeDetail.tsx"
...
def _parse_frontend_targets() -> set[str]:
    ...
    assert m, f"Could not find PROTECTED_TARGETS Set in {FRONTEND_FILE}"
    body = m.group(1)
...
def test_protected_targets_in_sync():
    ...
    assert backend == frontend, (
        ...
        "Update NodeDetail.tsx to match fleet_platform/models/pending_action.py"
    )


def test_frontend_has_is_protected_target_helper():
    """NodeDetail.tsx must define the isProtectedTarget helper function."""
    src = FRONTEND_FILE.read_text()
    assert "isProtectedTarget(" in src, "NodeDetail.tsx must contain the isProtectedTarget() helper function"
