...
# ---------------------------------------------------------------------------
# #745 — blocking Celery .get() in nodes.py /vms must use asyncio.to_thread
# ---------------------------------------------------------------------------


def test_nodes_vms_does_not_use_run_in_executor():
    """#745: list_node_vms must NOT call loop.run_in_executor — use asyncio.to_thread instead."""
    from pathlib import Path

    src = (Path(__file__).resolve().parents[2] / "fleet_platform/api/routes/nodes.py").read_text()

    fn_start = src.find("async def list_node_vms(")
    assert fn_start != -1, "list_node_vms function not found in nodes.py"
    fn_end = src.find("\n@router.", fn_start + 1)
    fn_src = src[fn_start:] if fn_end == -1 else src[fn_start:fn_end]

    assert "run_in_executor" not in fn_src, "list_node_vms still uses run_in_executor; switch to asyncio.to_thread"


def test_nodes_vms_uses_asyncio_to_thread():
    """#745: list_node_vms must use asyncio.to_thread for the blocking Celery wait."""
    from pathlib import Path
    ...
    assert "asyncio.to_thread" in fn_src, "list_node_vms must use asyncio.to_thread for the blocking .get() call"


@pytest.mark.asyncio
async def test_nodes_vms_parse_tart_output_still_works():
    """#745: _parse_tart_output helper continues to work correctly after the fix."""
    from fleet_platform.api.routes.nodes import _parse_tart_output

    vms = _parse_tart_output('[{"name":"vm1","state":"running"}]')
