...
// plus conflict/no-secret warnings (#702).
// ---------------------------------------------------------------------------

function ResolvedCredentialPanel({ nodeId }: { nodeId: string }) {
...
function ConnectivityPanel({ node, canManage }: { node: NodeDetailData; canManage: boolean }) {
    ...
    <div className="space-y-6">
      ...
      {tab === 'overview' && (
        <div role="tabpanel" id="tabpanel-overview" aria-labelledby="tab-overview" className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <ConnectivityPanel node={node} canManage={currentUser?.role === 'admin' || currentUser?.role === 'operator'} />
