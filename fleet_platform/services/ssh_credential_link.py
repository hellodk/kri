...
async def owner_secret_flags(
    ...
    *,
    ...
    Resolution goes solely through the linked ``Credential`` (#748 — ARC-4, #913).
    An owner with no ``credential_id`` (or a dangling FK) has no stored secret,
    so this returns ``(False, False)``.
    """
