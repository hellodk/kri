...
def bootstrap_node(self, node_id: str, target_ip: str, ssh_username: str | None = None) -> dict:
    ...
    logger.info("bootstrap_node: pillar written for minion_id=%s node_id=%s", node_minion_id, node_id)
    ...
    try:
        ...
        # #445 Part A: if step 6 never completed (SoftTimeLimitExceeded or other exception),
        # mark any still-running BootstrapRun row as failed so it is not left as an orphan.
