...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_ansible_jobs_id_text ON ansible_jobs (cast(id AS text) text_pattern_ops)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_execution_jobs_id_text ON execution_jobs (cast(id AS text) text_pattern_ops)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_llm_query_log_id_text ON llm_query_log (cast(id AS text) text_pattern_ops)"
    )
