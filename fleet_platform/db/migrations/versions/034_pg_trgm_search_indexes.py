...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_ansible_jobs_playbook_trgm ON ansible_jobs USING gin (playbook gin_trgm_ops)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_ansible_jobs_target_label_trgm ON ansible_jobs USING gin (target_label gin_trgm_ops)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_llm_query_log_prompt_trgm ON llm_query_log USING gin (prompt gin_trgm_ops)"
    )
