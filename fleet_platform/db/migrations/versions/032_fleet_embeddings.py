...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX fleet_embeddings_tsv_idx "
        "ON fleet_embeddings USING gin (tsv)"
    )
    op.create_index(
        'idx_fleet_embeddings_source', 'fleet_embeddings',
        ['source_type', 'source_id']
    )
    op.create_index(
        'idx_fleet_embeddings_hash', 'fleet_embeddings', ['content_hash']
    )
