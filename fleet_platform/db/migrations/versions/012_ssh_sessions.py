...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('session_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ssh_sessions.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table(
        'security_events',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('session_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ssh_sessions.id', ondelete='SET NULL'), nullable=True),
